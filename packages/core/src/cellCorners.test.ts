...
import { cellCorners } from "./cellCorners.js";
import { cellKey } from "./cellKey.js";
import { faceVertices } from "./icosahedron.js";
import { pentagonVertex } from "./neighbour.js";
...
describe("cellCorners", () => {
	...
	it("is an exactly regular hexagon before it is projected", () => {
		...
		const flatPoint = (I: number, J: number) => {
			const wa = 3 * n - I - J;
			return {
				x: A.x * wa + B.x * I + C.x * J,
				y: A.y * wa + B.y * I + C.y * J,
				z: A.z * wa + B.z * I + C.z * J,
			};
		};
		...
		for (let f = 0; f < 20; f++)
			for (let i = 0; i <= n; i++)
				for (let j = 0; i + j <= n; j++) {
					...
					const nrm = cross(sub(c[1]!, c[0]!), sub(c[2]!, c[0]!));
					expect(dot(nrm, c[0]!)).toBeGreaterThan(0);
				}
	});
});
