...
export class PlanetSettings {
	...
	biomeOptions(): BiomeSettings {
		...
		return {
			...
			// **What the grid's height axis is a share of.** Its edges are
			// shares rather than metres, because the other three axes are
			// curves' answers and mean the same thing on every planet while
			// a metre does not: measured over three reliefs, land divides
			// into near-thirds at the same two shares and at no two metre
			// lines.
			groundTop: this.maxElevation,
