		...
		knobs: [
			{
				...
				digits: 1,
				enabledWhen: (k) => k.caves,
				says: "How much rock is always left between the ground and the top of a cave. Nothing opens inside it, so a passage running near the surface has its roof left on rather than dropping the ground out from under a player. It is not the depth caves start at: the passages are one folded sheet weaving through the whole crust at every depth, and this only says how close to daylight one is allowed to open.",
			},
			{
				key: "caveVary",
				label: "Roof lifts by up to",
				digits: 0,
				enabledWhen: (k) => k.caves,
				says: "How much of that rock is given back, in the places the next two knobs pick out. This is what gives a cave a way in: when the lift reaches the rock kept over the roof, the roof reaches daylight and the sheet opens where it happens to be there. Below that it only thins the rock and no mouth appears; well past it the ground opens across the whole dip at once. At zero the roof is the same everywhere and no world holds an entrance.",
			},
			{
				key: "caveRare",
				label: "Mouth rarity",
				...
				says: "How much of the world gets a lift at all. Low, the roof rises over half the ground and the caves are open everywhere; high, a mouth here and there. It cannot be left to a standard deviation: over a patch a few hundred metres across the field never sees its own full range, so a borrowed figure lifts the roof nearly everywhere again.",
			},
			{
				key: "caveMouthScale",
				label: "Mouth size",
				digits: 0,
				enabledWhen: (k) => k.caves && k.caveVary > 0,
				says: "How wide one lift is, in metres of ground, and so how far it is from one to the next. Small is a pothole, large is a whole valley sinking toward the caves. Only the deepest part of a lift actually breaks through, so the mouth itself is smaller than this.",
			},
		],
	},
	{
