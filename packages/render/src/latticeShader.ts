 ...
 *
 * `colourMix` chooses what a pass is drawing. At 0 each cell keeps its own
 * colour, which is what the surface wants. At 1 the whole pass takes the tint,
 * which is what a translucent shell wants: a shell carrying per-cell colours is
...
fn vertexMain(
	...
	out.colour = mix(colour, u.tint.rgb, u.colourMix);
...
fn fragmentMain(in : VertexOut) -> @location(0) vec4f {
	...
	// A pass drawing its own colours is opaque and takes none of this.
	let glass = u.colourMix;
	let lit = in.colour * mix(shade, 0.55 + 0.45 * lambert, glass);
	let alpha = u.tint.a * mix(1.0, 0.25 + 0.75 * rim, glass);
	return vec4f(lit + in.colour * rim * glass * 0.5, alpha);
}
`;
