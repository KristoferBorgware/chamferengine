...
import { LATTICE_SHADER } from "./LATTICE_SHADER.js";
...
interface Pass {
	...
	/** 0 draws the cells' own colors, 1 draws the tint as a shell. */
	readonly colorMix: number;
}

/**
 * Draws the bare lattice: an opaque sphere of flat-colored cells, and a
...
export class LatticeRenderer {
	...
	render(frame: Frame): void {
		...
		for (const pass of this.passes) {
			...
			data.set(frame.viewProj.elements, 0);
			data.set(pass.tint, 16);
			data.set(frame.eye, 20);
			data[24] = pass.colorMix;
