...
/** Holdridge's own dots -- read by both `holdridge` and `elevation` below. */
 ...
 *
 * **`elevation` is the same zones, read with the air drying as well as
 * cooling as the ground rises.** `holdridge`'s own altitude band comes from
 * temperature alone; humidity never moves with height, so a wet reading can
 * still climb a mountain. This set is `HOLDRIDGE`'s own dots, unchanged --
 * the table names nothing new. What differs is {@link BiomeSettings.humLapse},
 * a term of the climate itself and not a second landform gate: the border it
 * draws is still one Voronoi diagram over one square, just a square a summit
 * reads further into.
 */
