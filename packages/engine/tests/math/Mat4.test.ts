...
function apply(mat: Mat4, p: readonly [number, number, number]) {
...
describe("lookAt", () => {
	...
	it("keeps the basis orthonormal", () => {
		const m = Mat4.lookAt([3, 4, 5], [0, 1, 0], [0, 1, 0]);
		const e = m.elements;
		const rows = [0, 1, 2].map((r) => [e[r]!, e[4 + r]!, e[8 + r]!]);
