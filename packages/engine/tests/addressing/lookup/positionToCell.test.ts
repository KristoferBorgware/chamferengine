...
describe("directionToCell", () => {
	...
	it("never lands further than a neighbour away from the nearest centre", () => {
		...
		for (let t = 0; t < samples; t++) {
			const dir = new Vec3(rnd() * 2 - 1, rnd() * 2 - 1, rnd() * 2 - 1);
			...
			let best = centres[0]!;
			let bestD = unit.sub(best.pos).length();
			for (const c of centres) {
				const d = unit.sub(c.pos).length();
				if (d < bestD) {
					bestD = d;
					best = c;
				}
...
describe("positionToCell", () => {
	it("ignores the radius", () => {
		...
		const near = positionToCell(p.scale(1700), n);
		const far = positionToCell(p.scale(1_000_000), n);
