...
### F-119 — A biome cannot be much wider than the terrain's widest layer
...
**Effort:** medium
**Found:** 2026-08-31, asked why deep caves kill the planet view
**Closed:** 2026-08-31. `CAVE_DETAIL_REACH` is `1`: the mesh worker hands its
coarse generators options with `caves` off, the seat `CUTOUT_REACH` already
gates the leaf holes from, and the generator itself stays level-blind.
Measured on the same land at every level with caves to 200 m
(`tools/trial-cave-lod.ts`): generation runs about `2x` a caveless chunk at
every level, and triangles run `15x` at the finest level down to `2.9x` at
32 m blocks -- so the three quarters of a standing player's 358 chunks that
are coarser than the finest two levels were paying the full walk to draw
noise. Frames of one mountainside both ways: the near caves stay, the
mid-distance slopes lose their swiss-cheese pocking, and no seam opens at a
level join. What is left of the deep-cave bill is the finest levels' own
sealed pockets, which is F-146.
**Where:** `packages/engine/src/mesh/worker/MeshWorkerCore.ts`,
`packages/engine/src/mesh/CAVE_DETAIL_REACH.ts`
...
**Closed:** 2026-08-31 on `master` by "Give the landform grid a height axis,
and take the peak rule back out." The fourth axis is the fix this entry
names, and `peakHeight` -- the rule that prompted it -- is gone with it,
because two answers to one question is what the entry was about. The axis
is a share of the ground the world reaches rather than metres, for the
reason the entry's own measurements gave.

**The grid is authored to build the world it already built**, which is the
finding restated as a result: five of the fifty-four cells differ from the
old grid spread across the new axis, and the six landforms measure the same
shares to the digit. The three shape axes already correlated with height on
this grid, and that correlation was the whole reason the fault was invisible.
What is different is that a peak is now *required* to stand in the top band
rather than happening to, and the requirement is a digit a reader can see and
move rather than a condition after the lookup.

The near-the-sea sheet is left not using the axis on purpose: its sharp column
tops out at slopes in every height band, which is the old grid's reasoning
that a coast is usually low. Turning one digit gives a high coast its range,
and that is a change to the world rather than a correction, so it is offered
rather than taken.
...
**Closed:** 2026-08-31 on `master` by "Show every ground in the table, and
keep a bench's own busy line in a row." The list shows all twenty-one
whatever chip is selected: the ones this landform can build first, then a
rule reading `on other ground` and the rest dimmed, each with the landform
that owns it where its share would be. Clicking one moves to that landform
rather than picking a dot the diagram beside it does not draw, which is
also how a reader learns the rule. The diagram is untouched -- it is a
picture of one landform's own Voronoi, and a dot that is not in it would be
a lie.
...
on, and hold its square fixed." Measured first, as this entry's own fix
asked: with `fit` on, six sample raw climate readings put through two
different planets' own fits named a different Holdridge zone for two of the
six, purely from the stretch. `biomeOptions()` now sets `fit` from the
`biomeFit` knob only when the live table's preset is `"plain"`, and to
`false` for every other table -- `holdridge` and `elevation` both, since
both now name a real classification. The panel disables "Fit all biomes on
the planet" to match, rather than leaving it live and inert.
...
**Closed:** 2026-08-30 by the third answer below rather than by the shell.
One face a boundary, `cullMode: "none"` on the cutout pipeline, and the owner
settled on the lower canonical name -- except against solid, where the solid
owns it, because a look through a leaf at a trunk has to find something it
cannot see through. **1.37x to 1.20x** over a real view, 277.2 MB to 244.7,
and the picture is the same to **0.01 of 255** outdoors with no percentile
from 5th to 95th moving inside a canopy. The shell is left unbuilt: it reaches
`1.06x` and gives up a canopy's depth from inside, and this takes half of that
saving for nothing at all.
...
all -- and the one that shipped.** Every leaf-to-leaf boundary was drawn
**twice**, once from each side, and back-face culling showed exactly one of
the two: two sets of vertices to rasterise exactly as many fragments. One face
per boundary, drawn from both sides, is the same picture for half the interior
geometry.

> **[measured]** Over the same view, **1.20x** the triangles rather than
> `1.37x` and **244.7 MB** rather than 277.2. Outdoors the two are the same to
> **0.01 of 255** over 447,495 pixels, fifth and ninety-fifth percentiles both
> `1.000`; inside a canopy the mean moves **0.78 of 255** with both those
> percentiles still `1.000`, so nine pixels in ten do not move at all.

What it gives up is that a face carries its owner's corner shading and sky
exposure rather than each side's own, which is the tail that moves inside a
canopy. **The owner cannot be arbitrary**: against solid it has to be the
solid, whose face has no holes in it, or a look through a leaf at a trunk sees
through the trunk. Between two cutouts it is the lower **canonical** name --
canonical because a cell on a face edge has several, and two chunks comparing
different names for one point would both draw the face or neither would.
...
**Closed:** 2026-08-28, fixed, and **the exact route was taken rather than the
stride**. `carveMargin` hands back the sum `carveIsRock` compares, and
`carveStep` bounds how far it can move between one block of a column and the
next -- the field's own steepest slope along the ray, the waterline term's, and
the depth ramp's, added. A margin further from nought than `k` times that is
the same answer for `k` blocks, proved rather than assumed. Measured over the
same `8,580` columns: **90.3 readings a column against 120**, `25%` fewer, and
**0** cells different. A chunk of ground goes **`138.0 ms` to `116.2 ms`**.

**The stride would have been faster and is not worth it**: 62.9 readings and
`101` cells wrong, at the surface. **And tightening the bound per column is
not worth it either** -- weighting the three axes by how much of its own
direction a column carries takes it to `86.2` readings, `4.5%`, and the
arithmetic costs at least that back.
**Where:** `packages/engine/src/generation/terrain/TerrainGenerator.ts`
...
gone, and **nothing ends the forest**: a plant between half a block and a whole
one is floored at one leaf block, which is the tree rounded to the grid, and
under half a block it stops being a shape and becomes the colour of the ground
it stands on (F-117). Over one 2,048 m triangle the share of ground under a
canopy runs `22.3% / 20.5% / 21.2% / 22.2% / 22.9% / 23.4%` at levels 0 to 5,
where level 5 drew **nothing at all** before.
...
disappearing
**Closed:** 2026-08-29, fixed. A plant under half a block now marks its column
in a `cover` map instead of growing, and the mesher paints that column's
**up-facing ground cap** with the canopy's own colour. Only that cap -- a cliff
face under a forest is still rock -- and **no block is written**, so nothing a
player stands on, breaks or collides with moves, no geometry is added and
nothing is stored. It works at any block size, so the forest has no last level.

**What it cost was a bound on the root scan, and that bound turned out to be
about the picture rather than about the cost.** A column asks the block of
`4^lod` roots it covers, which grows without limit; capped, it asks `12 x 12`
of them. **Asking more makes the picture worse**: a column is green or it is
not, so every extra root asked is another chance to paint a whole column green
for one tree in it. Measured over one 4,096 m triangle, canopy runs `22.9%` at
a level whose whole block fits inside the cap, and the coarse levels come out
`17.0-18.6%` at a sample of 8 a side, **`24.0-25.6%` at 12**, `28.0-29.8%` at
16 and `35.0-39.5%` at 32 or 64. The sample is the block's own corner, which is
what keeps the nesting exact: whatever a column samples, its four children
between them sample all of, so nothing a player walks toward vanishes.

Growing a chunk's plants now runs `730 / 238 / 140 / 85 / 40 / 35 / 38 ms` at
levels 0 to 6 against the ground's own `111 / 58 / 30 / 28 / 14 / 7 / 4 ms` --
falling with distance and then flat, never climbing, where the uncapped scan
read `129 ms` at level 6 and would have quadrupled at each level past it. The
opening frame moves `0.08` of `255`.
...
**Closed:** 2026-08-30, declined -- the standalone labs (multi-noise, biomes,
vegetation, caves) are deprecated now that the client benches cover the same
ground, live, against the real engine rather than a copy of it. Building a
biome stand-in into a lab that is being retired spends effort on the side of
the line that is going away; `demos/vegetation-lab.html` and its digest test
are left as they are rather than extended.
...
**Why it matters.** Little, given the closing reason above -- recorded so the
gap is not rediscovered as new before someone remembers the lab is retired.

**What would fix it.** Not this. If the labs outlive their deprecation for some
reason, either give the lab a small biome-membership stand-in (even a fixed
checkerboard, the way the growStand tests use one) and a layer control for it,
matching the engine's mechanism closely enough to digest-check the planting
logic itself rather than only the three shared primitives; or note in the
lab's own text that biome restriction is engine-only and the lab cannot
preview it.
...
rebuild
**Closed:** 2026-08-30, fixed. A chunk `dropChunk` takes out of `drawn` goes
into `retiring`, which is the set that owns a mesh still drawing that this
file has stopped counting on -- so `dropReplaced` and `trimRetired` reach it
the way they reach everything else. Two rules had to move with it. An arrival
leaves `retiring` as it joins `drawn`, or `dropReplaced` would drop the mesh
that had just been uploaded, that chunk being the one covering its own ground.
And a retiring chunk is un-retired only when it is **drawn** again rather than
merely wanted again -- until the rebuild lands it is in neither, which is the
state that had no owner. `retiring` was a subset of `drawn` before, so that
guard is a no-op on every path that existed.

**Measured, and it does not show the leak.** Six breaks and a walk away in the
real client, resident chunks against the selection: `316 / 319 / 320 / 320 /
320 / 320 / 320` with the fix and `314 / 318 / 319 / 319 / 319 / 319 / 319`
without it -- the same shape, because the player stays beside the ground they
broke and the chunk is re-selected and uploaded over either way. What the two
runs show is that the common path did not move; the leak needs the edited
ground to leave the selection and never come back, which the harness cannot
drive without a flight long enough to swamp a one-chunk signal. What the fix
rests on instead is the enumeration: every `drawn.delete` in the file now sits
beside a `renderer.drop` or a `retiring.add`, and there are four of them.
