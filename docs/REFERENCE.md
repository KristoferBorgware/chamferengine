...
2. the blind spot costs a POINT QUERY, not a chunk
   ...
   this machine, now: 436 ns -- a timing, so it moves run to run
...
5. is the garbage collector the discriminator? (wall-clock, read ratios)
   ...
   (b) the mesher -- building doc 14's 84,000-triangle buffer, per rebuild
       ...
       This machine, now: typed arrays 0.93 ms, one object a vertex
       9.11 ms -- a layout gap of 10x. Both are timings and move run to
...
5. drainage area, and what counts as a river
   ...
   whole pass: well under a second for 163,842 cells  (this run 1303 ms -- a timing, so it moves run to run)
