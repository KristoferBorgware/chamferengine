...
1. how ragged the coastline is, and how that changes with resolution
   ...
   5         11.72   (781)  12.51   (827)  20.44  (1007)  21.42  (1319)
   6         12.18  (1623)  14.11  (1865)  27.00  (2655)  25.15  (3143)
   7         13.23  (3523)  15.46  (4087)  36.78  (7239)  33.22  (9925)
   ...
   plate  x2.38 then x3.16   dimension 1.25 then 1.66
...
2. what each one does to the land itself
   ...
   plate  land 30.0%  largest landmass  34359  islands  102
...
4. how long a river gets, which is what the land allows
   ...
   plate  longest river  114 cells  on a landmass of  34359
   ...
   others. The plate field has the largest landmass of the four and still
   runs a shorter river than either noise field, so there the limit is the
   ground rather than the coast -- a range raised along every seam cuts the
   interior into basins, and a river runs from a ridge to the nearest coast
   instead of across the continent.
...
verdict
   ...
   Plates give up neither -- the fraction is exact and the landmass is the
   largest of the four -- and lose the rivers a different way, by raising a
   range along every seam and cutting the interior into basins.
...
5. sharing an exact location
   D=11: address 29 bits -> 6 chars,  +11-bit layer 40 -> 8 chars,  +12-bit planet 52 -> 11 chars
   D=13: address 33 bits -> 7 chars,  +11-bit layer 44 -> 9 chars,  +12-bit planet 56 -> 11 chars
   So an exact, lossless "here" inside one world is EIGHT base-36
   characters, and ELEVEN if the code has to say which planet too.
...
verdict
   ...
   which is 40 bits with its layer -- eight base-36 characters, or eleven
...
4. what the options actually are
   ...
   whichever wins, the word at D 11 with a 12-bit planet field:
     planet 12 + address 29 + layer 11 = 52 of 64, 12 spare
...
verdict
   ...
   ADDRESS = 5 + 2D + 2 bits.  WORD = [planet 12][address 29][layer 11]
   = 52 of 64 at D 11, 12 spare, 4,096 worlds of 41,943,042 cells.
```
...
2. max crust in layers = (1-t) * 2^D / K  -- the radius cancels
   ...
   13       0.250 m             1741               435 m   taper binds
   14       0.125 m             3482               435 m   layer field binds (2048)
...
4. what merging layers would buy, as a fraction of the radius
   ...
   But the ID gives the layer 11 bits, which addresses 2048 layers (docs 03, 06).
   Unmerged reach is 435 layers; the field stops at 2048. So the first merge
   buys 1613 addressable layers -- 371% more crust -- and every merge after
...
verdict
   buys : 1613 layers of addressable crust on the worked planet, 371%
